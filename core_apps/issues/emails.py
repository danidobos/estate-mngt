...
def send_issue_confirmation_email(issue: Issue) -> None:
    try:
        ...
        html_email = render_to_string('emails/issue_confirmation.html', context=context)
...
def send_issue_resolved_email(issue: Issue) -> None:
    try:
        ...
        html_email = render_to_string(
            'emails/issue_resolved_notification.html', context=context
        )
        text_email = strip_tags(html_email)
        from_email = DEFAULT_FROM_EMAIL
        to = [issue.reported_by.email]
...
def send_issue_resolution_email(issue: Issue) -> None:
    try:
