    ...
    def perform_create(self, serializer: IssueSerializer) -> None:
        ...
        try:
            ...
            raise PermissionDenied(
                'You do not have permission to report an issue for this apartment. It is not yours.'
            )
...
class IssueDeleteAPIView(generics.DestroyAPIView):
    ...
    def get_object(self) -> Issue:
        ...
        if not (user == issue.reported_by or user.is_staff):
            logger.warning(
                f'Unauthorized delete attempt from user {user.get_full_name} on issue {issue.title}'
            )
