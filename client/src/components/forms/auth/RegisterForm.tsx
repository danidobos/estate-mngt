...
import { Contact2Icon, MailIcon, UserCheckIcon } from "lucide-react";
...
export default function RegisterForm() {
	...
	const onSubmit = async (values: z.infer<typeof registerUserSchema>) => {
		try {
			...
			toast.success(
				"An email with an activation link has been sent to your email address. Please check your email and activate the account.",
			);
			router.push("/login");
			reset();
		} catch (error) {
		...
		<main>
			...
			>
				<FormFieldComponent
					...
					startIcon={<UserCheckIcon className="dark:text-babyPowder size-8" />}
				/>
