				...
				return (
					<SheetClose asChild key={linkItem.path}>
						...
						>
							...
							/>
							<p className={`${isActive ? "base-bold" : "color-medium"}`}>
								{linkItem.label}
							</p>
						</Link>
					</SheetClose>
						...
						<SheetFooter>
							{isAuthenticated ? (
								<Button
									...
									className="lime-gradient small-medium light-border-2 btn-tertiary text-text-baby_richBlack mt-4 min-h-[41px] w-full rounded-lg border px-4 py-3 shadow-none"
								>
									Log Out
								</Button>
							) : (
